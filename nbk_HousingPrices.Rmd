...
XGBoost is a supervised machine learning technique, where the output is provided.  
...
for (x in field.attributes$integer)
{
  data.test[[x]][is.na(data.test[[x]])] <- -1
}

dtest=xgb.DMatrix(as.matrix(data.test[,field.attributes$integer]))
...
predict(model.xgboost,dtest)

xgb.plot.tree(feature_names =field.attributes$integer, model=model.xgboost)

importance_matrix <- xgb.importance(feature_names =field.attributes$integer, model=model.xgboost)
