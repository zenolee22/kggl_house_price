...
```{r}
df <- data.train %>% group_by(Neighborhood) %>% summarize(Med_SalePrice=median(SalePrice)) %>% arrange(Med_SalePrice) 
...
plot_ly(type="bar"
  ...
  layout(title="Neighborhoods by Median Sale Price")
```
...
mlrn.ctrl <- makeTuneControlRandom(maxit = 200)  # Optimization Algorithm
