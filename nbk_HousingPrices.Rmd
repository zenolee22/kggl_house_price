...
# Some packages are not available for R 3.5
...
For any numeric variable, let's replace anything missing with 0.  
...
anchor.train.encoded <- anchor.encoded[1:1458,]
anchor.test.encoded <- anchor.encoded[1459:2917,]
...
dtrain <- xgb.DMatrix(as.matrix(anchor.train.encoded %>% dplyr::select(-SalePrice)),label=response.train)
...
mlrn.params <- makeParamSet( 
  ...
  ,makeNumericParam("min_child_weight",lower = 3,upper = 20)
...
mlrn.ctrl <- makeTuneControlRandom(maxit = 100)  # Optimization Algorithm
...
write.csv(prediction,  paste0(dir.data,"train_results_5.csv"))
