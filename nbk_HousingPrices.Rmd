...
No need to analyze columns that are all NULL, so let's remove them from the database.

```{r rmv_Nulls, eval=TRUE, message=FALSE, warning=FALSE}
#Copy the Train Data and Remove what we already don't need
anchor.train <- data.train %>%
  select(-SalePrice,-Alley,-PoolQC,-Fence,-MiscFeature,-FireplaceQu,-LotFrontage)

# We remove SalePrice because we don't want the One Hot Encoding to consider it as a feature
response.train <- data.train$SalePrice

anchor.test <- data.test %>% select(-Alley,-PoolQC,-Fence,-MiscFeature,-FireplaceQu,-LotFrontage)
#response.test <- data.test$SalePrice
```
...
field.features <- setdiff(names(anchor.train),c(field.ID,field.Target))
...
#rmvitems <- c("PoolQC","MiscFeature","Alley","Fence","FireplaceQu","LotFrontage")
...
boruta.train <- anchor.train %>% select(-Id)
...
What we are looking for are variables where the distribution is skewed.  Depending on the distribution, we'll have to determine how to handle it.

  1.  Remove the feature from the analysis    
  2.  Bin categories together
  3.  Impute missing data with mean/median values
...
anchor.train %>% group_by(Street) %>% summarize(Count=n()
                                                ,Pct=n()/nrow(anchor.train))

```

There are only two variables with no opportunity to do anything to improve the significance of the variable.
...
anchor.train.encoded <- cbind(anchor.train.encoded, response.train)
...
```{r Consistent_Data, eval=TRUE}
#Use Only Columns that are available in Test after 
anchor.train.encoded.cleaned <- anchor.train.encoded[colnames(anchor.train.encoded) %in% colnames(anchor.test.encoded)]


#Filter out anything not used by Train
anchor.test.encoded.cleaned <- anchor.test.encoded[colnames(anchor.test.encoded) %in% colnames(anchor.train.encoded.cleaned)]

anchor.train.encoded.cleaned <- cbind(anchor.train.encoded.cleaned, response.train)
colnames(anchor.train.encoded.cleaned)
```


```{r XGBoost_Default, eval=FALSE}

dtrain <- xgb.DMatrix(as.matrix(anchor.train.encoded.cleaned),label=response.train)
dtest <- xgb.DMatrix(as.matrix(anchor.test.encoded.cleaned))
...
#blah <- anchor.train.encoded[colnames(anchor.train.encoded) %in% colnames(anchor.test.encoded)]
#blah2 <- anchor.test.encoded[colnames(anchor.test.encoded) %in% colnames(blah)]

#blah <- subset(anchor.train.encoded,select=c(OverallQual,GrLivArea,TotalBsmtSF,response_train))


mlrn.traintask <- makeRegrTask(data=anchor.train.encoded.cleaned,target="response.train")

#mlrn.traintask <- makeRegrTask(data=anchor.train.encoded,target="response_train")
...
mlrn.params <- makeParamSet( 
  ...
  ,makeIntegerParam("gamma",lower=0,upper=25)
...
mlrn.rdesc <- makeResampleDesc("CV",iters=10)  # Performs n-fold cross validation
mlrn.ctrl <- makeTuneControlRandom(maxit = 30)  # Optimization Algorithm
...
anchor.test.encoded.cleaned$response.train <- 0
...
mlrn.testtask <- makeRegrTask(data=anchor.test.encoded.cleaned,target="response.train")
...
model <- train(mlrn.tuned_param, mlrn.traintask)
...
write.csv(prediction.tuned,  paste0(dir.data,"final_7.csv"))
```
